...
namespace ConversionBack
{
    public class ConvertBack
    {
        ...
        public static List<Tuple<OpCode,object>> Runner(int position, int size, int ID, object[] parameters, MethodBase meth)
        {

            {
                MethodBase callingMethod = meth;// new StackTrace().GetFrame(1).GetMethod();//get calling method, using this can prevent invocation and dynamic unpacking
                ...
                var decrypted = Decrypt(decryptionKey, grabbedBytes);
                 ConversionBack(decrypted, ID, parameters, callingMethod);//if not convert the method to a dynamic method
                return to_return;
            }
        ...
        public static List<Tuple<OpCode, object>> to_return = new List<Tuple<OpCode, object>>();

        public static void ConversionBack(byte[] bytes, int ID, object[] parameters, MethodBase callingMethod)
        {
            to_return.Clear();
            ...
            for (var i = 0; i < instructionCount; i++)
            {
                ...
                {
                    opc = Initialize.oneByteOpCodes[opcode];
                }
                else
                {
                    var b2 = (byte)(opcode | 0xFE00);
                    opc = Initialize.twoByteOpCodes[b2];
                }
        ...
        {
            ConvertBack.to_return.Add(new Tuple<OpCode, object>(opcode, null));
        }
        ...
        private static void InlineMethodEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader)
        {
            ...
            {
                ConvertBack.to_return.Add(new Tuple<OpCode, object>(opcode, (MethodInfo)resolvedMethodBase));

            }
            else if (resolvedMethodBase is ConstructorInfo)
            {
                ConvertBack.to_return.Add(new Tuple<OpCode, object>(opcode, (ConstructorInfo)resolvedMethodBase));
            }
            else
        ...
        private static void InlineVarEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader, List<LocalBuilder> allLocals)
        {
            ...
            if (parOrloc == 0)
            {
                ...
                ConvertBack.to_return.Add(new Tuple<OpCode, object>(opcode, label));
            }
            else
            {
                ConvertBack.to_return.Add(new Tuple<OpCode, object>(opcode, index));
        ...
        private static void InlineStringEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader)
        {
            ...
            ConvertBack.to_return.Add(new Tuple<OpCode, object>(opcode, readString));
        }
        ...
        private static void InlineFieldEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader)
        {
            ...
            ConvertBack.to_return.Add(new Tuple<OpCode, object>(opcode, fieldInfo));
        }
        ...
        private static void InlineTypeEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader)
        {
            ...
            ConvertBack.to_return.Add(new Tuple<OpCode, object>(opcode, type));
        }
        ...
        private static void ShortInlineIEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader)
        {
            ...
            ConvertBack.to_return.Add(new Tuple<OpCode, object>(opcode, b));
        }
        private static void ShortInlineREmitter(ILGenerator ilGenerator, OpCode opCode, BinaryReader binaryReader)
        {
            ...
            ConvertBack.to_return.Add(new Tuple<OpCode, object>(opCode, myFloat));
        }
        private static void InlineREmitter(ILGenerator ilGenerator, OpCode opCode, BinaryReader binaryReader)
        {
            ...
            ConvertBack.to_return.Add(new Tuple<OpCode, object>(opCode, value));
        }
        ...
        private static void InlineSwitchEmitter(ILGenerator ilGenerator, OpCode opCode, BinaryReader binaryReader, Dictionary<int, Label> _allLabelsDictionary)
        {
            ...
            ConvertBack.to_return.Add(new Tuple<OpCode, object>(opCode, allLabels));
        }
        ...
        private static void InlineTokEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader)
        {
            ...
            else if (type == 1)
            {
                ...
                ConvertBack.to_return.Add(new Tuple<OpCode, object>(opcode, typeInfo));
            }
            else if (type == 2)
            {
                ...
                {
                    ConvertBack.to_return.Add(new Tuple<OpCode, object>(opcode, methodinfo));
                }
