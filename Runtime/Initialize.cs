...
namespace ConversionBack
{
    public class Initialize
    {
        ...
        public static void Initalize(string resName, Module module)
        {

            callingModule = module;

            byteArrayResource = extractResource(resName, module);
            byte[] tester = extractResource("Eddy^CZ", module);
            ...
            if (IntPtr.Size == 4)
            {
                byte[] tester2 = extractResource("X86", module);
            ...
            {
                byte[] tester2 = extractResource("X64", module);
                EmbeddedDllClass.ExtractEmbeddedDlls("NativePRo.dll", tester2);
                abb = EmbeddedDllClass.LoadDll("NativePRo.dll");
                def = e(abb, "a");
            }
        ...
        private static byte[] extractResource(string resourceName, Module module)
        {
            using (Stream stream = module.Assembly.GetManifestResourceStream(resourceName))
